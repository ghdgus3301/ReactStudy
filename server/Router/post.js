...
router.post("/submit", (req,res) => {
    ...
    Counter.findOne({name : "counter"}).exec().then((counter) => {
        ...
        User.findOne({uid : req.body.uid}).exec().then((userInfo) => {
            ...
            Counter.updateOne({name : "counter"}, {$inc : {postNum : 1}}).then(() => {res.status(200).json({success : true});
    });
        });
        });
    }).catch((err) => {
...
router.post("/delete", (req,res) => {
    Post.deleteOne({postNum : Number(req.body.postNum)}).exec().then((doc) => {
        res.status(200).json({success : true, post : doc});
    }).catch((err) => {
        res.status(400).json({success : false});
    })
...
router.post("/image/upload", setUpload("react-community2/post"), (req,res,next) => {
    console.log(res.req);
